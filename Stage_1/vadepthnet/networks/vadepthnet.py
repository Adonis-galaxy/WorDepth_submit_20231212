...
class VADepthNet(nn.Module):
    def __init__(self, pretrained=None, max_depth=10.0, prior_mean=1.54, si_lambda=0.85, img_size=(480, 640),hidden_dim=256,std_reg=1):
        ...
        self.loss = VD_Loss( self.max_depth)
    ...
    def forward(self, image, gts=None, text_feature=None):
        outs={}
        # x=None
        ...
        # room_feat=self.feat_up(feat)
        # # 1,256 -> 1,1024

        
        metric = self.mlayer(room_feat)
        # print("metric:",metric.shape)，metric: torch.Size([1, 2, 1, 1])

        # text_feature: torch.Size([1, 1024])
        room_feat=room_feat.unsqueeze(-1).unsqueeze(-1) # 1,1024,1,1

        d = self.vlayer(room_feat)
        # print(d.shape)，torch.Size([1, 128, 30, 40])
        ...
        # keep refine and upsample to 480 * 640
        
        d = torch.sigmoid(metric[:, 0:1]) * (self.outc(d) + torch.exp(metric[:, 1:2]))
        # d = self.outc(d)
        # d=1/d

        # Try to directly regress depth value, 

        outs['scale_1'] = d

        # Use edge padding in conv
        ...
        #     return outs['scale_1']
        if self.training:
            # loss = self.huber_loss(d, gts) + max(0,(1-std_norm))
            # loss = self.huber_loss(d, gts)
            loss, loss_depth, loss_smooth = self.loss(d,gts,image) 
            loss = loss + max(0,(self.std_reg-std_norm))
            return outs['scale_1'], loss, std_norm , loss_depth, loss_smooth
        else:
            return outs['scale_1']
