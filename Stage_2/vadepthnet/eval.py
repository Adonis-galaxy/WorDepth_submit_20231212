    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            for i in range(len(eval_sample_batched['sample_path'])): # B=4
                ...
                path_inter=eval_sample_batched['sample_path'][i].split(' ')[0][:-4]
                feature_path="../nyu_depth_v2/official_splits/test/"+eval_sample_batched['sample_path'][i].split(' ')[0][:-4]+'.pt'
                # feature_path="../nyu_depth_v2/sync/"+eval_sample_batched['sample_path'][i].split(' ')[0][:-4]+'.pt'
            ...
            # Pred Vis
            pred_depth = model(image, text_feature=text_feature_list)
            
            vis_depth = pred_depth.cpu().numpy().squeeze()
            vis_gt_depth = gt_depth.cpu().numpy().squeeze()

            
            vis_path="models/"+ args.model_name +"/"+path_inter.replace("/","_")+".png"
            vis_depth = vis_depth / 10 * 255
            cv2.imwrite(vis_path, vis_depth)

            # GT vis
            vis_gt_depth=vis_gt_depth / 10 * 255
            vis_gt_path="models/"+ args.model_name +"/"+path_inter.replace("/","_")+"_gt"+".png"
            cv2.imwrite(vis_gt_path, vis_gt_depth)      
